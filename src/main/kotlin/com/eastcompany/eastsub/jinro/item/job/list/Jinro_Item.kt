...
class Jinro_Item : SpecialItem() {
    override val tag = "jinrou_0"
    override val lore = listOf("対象を\uE501で殺害 \uE500 + \uE501でスキル切り替え")
    override val material = Material.STICK

    override val role = Role.JINRO
    override val displayName = role.roleName + "スキル 切り裂く \uE501で対象を殺害 \uE500 + \uE501でスキル切り替え"
        ...
        if (currentItem.hasItemMeta()) {
            ...
            if (isCooltime == 1.toByte()) {
                ...
                if(player.isSneaking) {
                    player.inventory.setItemInMainHand(Roar_Item().createCooldownItem())
                }
                return
            }
            if(player.isSneaking) {
                player.inventory.setItemInMainHand(Roar_Item().create())
                return
            }
        }
